...
for(i in 1:4) {
  ...
  names(data)[grep(sample, names(data))] <- str_split_fixed(names(data)[grep(sample, names(data))], "\\.", 3)[,3]
...
DP <- ggplot(VCF_f, aes(x=DP, fill=SAMPLE)) + geom_density(alpha=.5, size=0.1) +
  geom_vline(xintercept=10, col="black") +scale_fill_manual(name="", values=col) + scale_y_continuous(expand=c(0,0)) +
...
save_plot("pileup/Diagnostics_DP_GQ.pdf", Plot_legend, base_height = 2, base_width = 6)
